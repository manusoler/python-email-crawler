...
import urlparse
import os
from settings import REMOVE_DDBB
...
class CrawlerDb:
    ...
    def connect(self):
        if REMOVE_DDBB and os.path.isfile(DATABASE_NAME):
    ...
    def enqueue(self, url, emails=None):
        ...
        if (emails != None):
            args = [{'url': unicode(url), 'has_crawled': True,
    ...
    def dequeue(self):
        ...
        s = select([self.website_table]).limit(1).where(
            self.website_table.c.has_crawled == False)
        res = self.connection.execute(s)
        result = res.fetchall()
        res.close()
        # If we get a result
